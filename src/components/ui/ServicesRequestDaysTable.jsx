...
const ServicesRequestDaysTable = () => {
  ...
  const fetchServices = async () => {
    ...
    try {
      const response = await getData("/admin/service-requests", {
        page: page,
        limit: limit,
        ...filters,
      });
  ...
  const handleConfirmDelete = async () => {
    try {
      await deleteData(`/admin/service-requests/${selectedItemId}`);
  ...
  const columns = [
    ...
    // ...columnsForLang,
    {
      ...
      cell: ({ row }) => {
        return `${row.original?.client?.user?.first_name} ${row.original?.client?.user?.last_name}`;
      },
    },
    {
...
export default ServicesRequestDaysTable;
