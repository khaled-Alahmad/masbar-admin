...
const AddAddAttributeValueModal = ({
  isOpen,
  itemId,
  onClose,
  refreshData,
}) => {
  // console.log("AddAddAttributeValueModal", itemId);
  ...
  const handleSubmit = async () => {
    ...
    Object.entries(formData.name).forEach(([lang, value]) => {
      data.append(`value[${lang}]`, value);
    });
    data.append("service_attribute_id", itemId);
    ...
    try {
      const response = await postData("/admin/service-attribute-values", data);
      if (response.success) {
...
export default AddAddAttributeValueModal;
