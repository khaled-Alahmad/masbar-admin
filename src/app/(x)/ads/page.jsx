...
const AdsTable = () => {
  ...
  const fetchServices = async () => {
    ...
    try {
      const response = await getData("/admin/ads", {
        page: page,
        limit: limit,
        ...filters,
      });
  ...
  const handleConfirmDelete = async () => {
    try {
      await deleteData(`/admin/ads/${selectedItemId}`);
      setIsDeleteModalOpen(false);
      toast.success("Deleted ads successful!");
      fetchServices();
    } catch (error) {
  ...
  const columns = [
    ...
    // },
    {
      ...
      accessorKey: "status",
      cell: ({ row }) => {
        if (row.original.status == "in_review") {
          return <span className={styles.pendingStatus}>In Review</span>;
